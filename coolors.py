    ...
    def execute(self, context):
        ...
        if active_material is not None:
            ...
            if active_material.node_tree.nodes.get("generated_coolors") is not None:
                color_ramp_node = active_material.node_tree.nodes.get("generated_coolors")
            else:
                color_ramp_node = active_material.node_tree.nodes.new(type='ShaderNodeValToRGB')
            ...
            for i in range(cols_len):
                if i != cols_len - 1:
                    element = color_ramp_node.color_ramp.elements.new((i / (cols_len - 1)))
                    element.color = cols_arr[i]
                else:
                    element = color_ramp_node.color_ramp.elements[len(color_ramp_node.color_ramp.elements)-1]
                    element.color = cols_arr[i]
            ...
            #active_material.node_tree.nodes.update()
...
def register():
    bpy.types.Scene.coolors_url = bpy.props.StringProperty(
        name="URL",
        description="Enter Coolors palette url"
    )
